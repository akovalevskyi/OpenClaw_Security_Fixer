...
## 🤖 5. Agent Monitoring & Output Sanitization
**Цель:** Контроль за поведением агента и предотвращение утечек через ответы.
- [ ] **Output Filtering:** Настроить регулярные выражения для блокировки ключей `sk-`, `ghp_` и персональных данных в ответах агента. *Автоматизировано в `security_fixer.py`.*
- [ ] **Indirect Injection Protection:** Убедиться, что инструменты получения данных (`web_search`, `fetch`) защищены Human-in-the-Loop (HITL) подтверждением. *Проверяется скриптом `security_audit.py`.*
- [ ] **System Logging:** Включить детальное логирование действий агента для последующего аудита. *Автоматизировано в `security_fixer.py`.*
- [ ] **System Instructions:** Использовать четкое разделение между системными инструкциями и пользовательским вводом в `systemPrompt`.
